...
public class MyAlgo2  implements ElevatorAlgo{
    ...
    private int findElevator(CallForElevator c) {
        ...
        for (int i = values.size() - 1; i >= 0; i--) {
            if (!values.get(i).isEmpty()) {
                return finalSerch(values.get(i), c);
            }
        }
        return 0;
    }
    ...
    private int finalSerch(ArrayList<Integer> list, CallForElevator c) {
        int ans = list.get(0);
        double minValue = calculateTime(ans, c.getSrc());
        for (int i = 0; i < list.size(); i++) {
            double iValue = calculateTime(list.get(i), c.getSrc());
            if (iValue < minValue) {
                ans = list.get(i);
                minValue = iValue;
            }
    ...
    private double calculateTime(int elev, int src) {
